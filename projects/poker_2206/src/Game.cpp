...
Game::Game()
    : human_player_(std::make_unique<HumanPlayer>("玩家")),
      ai_player_(std::make_unique<AIPlayer>("庄家")), human_wins_(0), ai_wins_(0), ties_(0)
{
}
...
void Game::replace_cards_phase()
{
    ...
    const HandEvaluation eval = HandEvaluator::evaluate(human_player_->get_hand());
...
void Game::showdown()
{
    ...
    // 比较结果
    const ComparisonResult result = HandComparator::compare(human_player_->get_hand(), // hand1
                                                            ai_player_->get_hand()     // hand2
    );
