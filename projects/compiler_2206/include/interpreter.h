 ...
 * 1. 预处理: 扫描源码建立行号索引
 * 2. 执行: 按行号顺序执行，遇到goto/if跳转
 *
 * 与编译器的区别:
 * - 解释器: 直接执行，支持动态数组索引、浮点运算
 * - 编译器: 生成SML码，受SML指令集限制
 *
 * 支持的完整功能:
 * - 浮点数运算
 * - 动态数组索引(如a(i)其中i是变量)
...
typedef struct {
    ...
    double step;           /**< 步长(可正可负) */
    int next_line_index;   /**< next语句后的行索引 */
...
typedef struct {
    ...
    /* ===== 变量存储 ===== */
    Variable variables[MAX_VARIABLES];  /**< 标量变量a-z */
    Array arrays[MAX_VARIABLES];        /**< 数组a()-z() */
